# Adaptation of the Matlab Code from Wang (2015) for structure learning (precision matrix and subgraph within subgroups)
# Update this code by including the subgraphs linking different subgroups and the variable selection 
# indicators gamma that depend on the complete graph through the MRF prior.

func_MCMC_graph <- function(sobj, priorPara, ini, S, method, MRF_2b){

  n    = sobj$n 
  p    = sobj$p  
  SSig = sobj$SSig 
  
  pii    = priorPara$pi.G   # prior probability of edge inclusion
  a      = priorPara$a      # hyperparameter in MRF prior
  b      = priorPara$b      # hyperparameter in MRF prior
  lambda = priorPara$lambda # hyperparameter in precision matrix prior
  V0     = priorPara$V0     # (p x p) matrix of small variances in prior of precision matrix for each subgroup
  V1     = priorPara$V1     # (p x p) matrix of large variances in prior of precision matrix for each subgroup
