...
WrapperSimCoxBVSSL = function(job, instance, model.type, ga.ini = 0, g.ini = 0,   
                        nu0 = 0.1, nu1 = 10, lambda = 1, a0 = -4, b0 = 1, ga.pi = 0.02,  
                        MRF2b = FALSE, b01 = 1, b02 = 1, n.iter = 20000, m.seed, ...){  
  ...
  data = instance$Traindata   # training data
  ...
  if(model.type == "Pooled"){ # for Pooled model combine/ pool data of all subgroups
    ...
    X.train = do.call( "rbind", lapply(data, function(x) x$X.train.unsc) )  # unscaled covariates
